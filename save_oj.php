...
if (isset($_POST['no_part']) and isset($_POST['kategori'])) {
    ...
    if ($part_no_mt == $qr_part_no[0]) {
        if ($now_date != $tgl_akhir and $kategori == 1) {
            // tambahkan ke stok area
            $add_stock_in = mysqli_query($conn, "INSERT INTO stock VALUES (NULL,'$qr_part_no[0]', 1, '$now_date', $last_stock, $qr_part_no[1], 0, $current_stock_in)");
        } else if ($now_date == $tgl_akhir and $kategori == 1) {
            ...
            // tambahkan ke stok area
            $add_stock_in = mysqli_query($conn, "INSERT INTO stock VALUES (NULL,'$qr_part_no[0]',$kategori,'$now_date','$last_stock', 0, $qr_part_no[1], $current_stock_out)");
            // update stock_area pada tgl terakhir
            ...
            // update stock_area pada tgl terakhir
            $add_stock_in = mysqli_query($conn, "UPDATE stock SET stock_out='$tambah_stock_out', current_stock='$current_stock_out' WHERE part_no='$qr_part_no[0]' and tgl='$tgl_akhir' and kategori='$kategori'");
            // update stock_area pada tgl terakhir
            $upd_stock_in = mysqli_query($conn, "UPDATE stock SET stock_in='$tambah_stock_in_wip', current_stock='$current_stock_in_wip' WHERE part_no='$qr_part_no[0]' and tgl='$tgl_akhir_wip' and kategori=1");
        }

        if ($now_date != $last_stock_all and $kategori == 1) {
            // tambahkan ke stok area
        ...
        } else if ($now_date == $last_stock_all and $kategori == 1) {
            // update stock_area pada tgl terakhir
